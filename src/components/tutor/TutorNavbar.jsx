...
function TutorNavbar({ activePage }) {
  const navigate = useNavigate();
  const [userId, setUserId] = useState('mock-tutor-id');
  const [showNotif, setShowNotif] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // Mock notifications state
  const [notifications, setNotifications] = useState([
    { id: 1, title: 'Lớp học mới', body: 'Bạn có một lớp học mới vào Thứ 2 tuần tới.', timestamp: new Date().toISOString(), isRead: false },
    { id: 2, title: 'Học phí', body: 'Học sinh Nguyễn Văn A đã đóng học phí tháng 3.', timestamp: new Date().toISOString(), isRead: true }
  ]);
  ...
  const navItems = [
    { label: 'Quản lý học sinh', to: '/students', key: 'students' },
    { label: 'Lớp học', to: '/classes', key: 'class' },
    { label: 'Lịch dạy', to: '/teaching-schedule', key: 'teaching-schedule' },
    { label: 'Giáo trình', to: '/syllabus', key: 'syllabus' },
    { label: 'Thu nhập', to: '/income', key: 'income' },
    { label: 'Trợ lý AI', to: '/ai-assistant', key: 'ai', badge: true },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button onClick={() => setShowNotif(!showNotif)} className="relative text-slate-500 hover:text-blue-500 transition-colors p-2">
            ...
            {showNotif && (
              <div className="absolute right-0 mt-3 w-80 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden z-[100]">
                <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                  ...
                  {unreadCount > 0 && (
                    <button onClick={markAllAsRead} className="text-[12px] text-blue-600 font-medium hover:underline">
                      Đánh dấu đã đọc
                    </button>
                  )}
                </div>
                <div className="max-h-[300px] overflow-y-auto custom-scrollbar">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map(notif => (
                      <div 
                        key={notif.id} 
                        onClick={() => { markAsRead(notif.id); setShowNotif(false); }}
                      ...
                      >
                        <div className="flex justify-between items-start gap-3">
                          ...
                          <div className="flex-1">
                            <p className={`text-[13px] ${!notif.isRead ? 'font-semibold text-slate-800' : 'text-slate-600'}`}>{notif.title}</p>
          ...
          <button onClick={handleLogout} className="hidden md:flex text-sm font-semibold text-red-500 hover:text-red-600 transition-colors items-center gap-2">
          ...
          <nav className="flex flex-col gap-2">
            {navItems.map((item) => (
              <Link
                ...
                className={`flex items-center justify-between p-3 rounded-xl transition-all ${activePage === item.key
              ...
              >
                <div className="flex items-center gap-3">
                   {item.label}
                   {item.badge && <span className="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></span>}
                </div>
